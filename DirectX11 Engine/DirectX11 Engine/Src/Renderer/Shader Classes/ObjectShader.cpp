...
bool ObjectShader::Render(int _indexCount, ID3D11RenderTargetView* _renderTarget)
{
...
void ObjectShader::Update(ChildMeshObject* _obj, ID3D11ShaderResourceView* _texture)
{
	if(_texture)
	{
...
bool ObjectShader::InitializeShader(int _vertexShaderIndex, int  _pixelShaderIndex)
{
...
bool ObjectShader::UpdateVertexShaderConstants(XMFLOAT4X4 _worldMatrix, XMFLOAT4X4 _viewProjMatrix)
{
	HRESULT hr;
...
bool ObjectShader::UpdatePixelShaderTextureConstants(ID3D11ShaderResourceView* _textureArray)
{
	if(_textureArray)
	{
...
{
	D3D11Renderer::d3dImmediateContext->OMSetRenderTargets(7, &D3D11Renderer::renderTargetView[1].p, D3D11Renderer::depthStencilView);
