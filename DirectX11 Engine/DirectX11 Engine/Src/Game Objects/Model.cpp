...
bool Model::Initialize(XMFLOAT3 _pos, XMFLOAT3 _scale, XMFLOAT3 _rotation, char* _modelFilename)
{
	bool result;
...
void Model::AddTexture(WCHAR* _filePath)
{
...
void Model::UpdateWorldMatrix(XMFLOAT3 _pos, XMFLOAT3 _scale, XMFLOAT3 _rotation)
{
	XMMATRIX mScale;
...
bool Model::LoadModel(char* _filePath)
{
	ifstream fin;
	char input;
	int i;
...
void Model::CalculateTangentBinormal(TempVertexType _vertex1, TempVertexType _vertex2, TempVertexType _vertex3, VectorType& tangent, VectorType& binormal)
{
...
void Model::CalculateNormal(VectorType _tangent, VectorType _binormal, VectorType& _normal)
{
	float length;
